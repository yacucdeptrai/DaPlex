...
**Luồng metadata** (control plane): Mọi thao tác CRUD trên nội dung, người dùng, vai trò, và cấu hình đều đi qua API REST. MongoDB lưu trạng thái bền vững; Redis2ndCache giảm latency cho các read-heavy operation (profile lookup, permission check). Access token HS256 15 phút + refresh token nanoid(32) 30 ngày (lưu trong Redis) bảo vệ toàn bộ control plane.
